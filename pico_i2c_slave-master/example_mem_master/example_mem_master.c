...
static void run_master(){
    for (uint8_t mem_address = 0;; mem_address = (mem_address + 32) % 256) {
        char msg[32];
        snprintf(msg, sizeof(msg), "Hello, I2C slave! - 0x%02X", mem_address); // the size put into the buffer is 32 (size of msg)
        uint8_t msg_len = strlen(msg); // Actual len of the string containing space. Should be 24?

        uint8_t buf[32]; //array
        buf[0] = mem_address; //first array element buf[0] is the mem_address
        memcpy(buf + 1, msg, msg_len); // put msg to the second element buf[1] with 
